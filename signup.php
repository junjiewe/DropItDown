...
$invalidPassword='<p><strong>Your password shoud be aat least 6 characters long and include a capital letter and one number!</strong></p>';
...
//if usernam exists in the users table print error
...
if($results){
    echo '<div class="alert alert-danger">That username is already register, Do you want to log in?</div>';
    exit;
}
...
$results = mysqli_num_rows($result);
if($results){
    echo '<div class="alert alert-danger">That email is already register, Do you want to log in?</div>';
    exit;
}
...
$message .= "http://junjiewe.thecompletewebhosting.com/DropSomethingDown/activate.php?email=".urldecode($email)."&key=$activationKey";
if(mail($email,'Confirm your Registration', $message, 'From:'.'junjiewe@buffalo.edu')){
